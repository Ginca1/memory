...
function Level17({ selectedLevel }) {
  const [font, setFont] = useState('');
  const [cards, setCards] = useState([]);
  const [flippedIndices, setFlippedIndices] = useState([]);
  const [matchedIndices, setMatchedIndices] = useState([]);
  const [score, setScore] = useState(0);
  const [time, setTime] = useState(115);
  ...
  useEffect(() => {
    const symbols = ['./img/book.webp', './img/dimants.webp', './img/emerald.webp', './img/gold.webp', './img/purple.png', './img/stone.webp', './img/sand.webp', './img/green.webp' , './img/birch.png' , './img/tnt.png'];
    const initialCards = [];
          ...
          <div className="card-container">
            <div className="row">
              {cards.map((card, index) => (
                <div
                  key={index}
                  className={`card2 ${flippedIndices.includes(index) ? 'flipped' : ''} ${matchedIndices.includes(index) ? 'matched' : ''}`}
